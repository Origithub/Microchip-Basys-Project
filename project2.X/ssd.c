...
extern int mode; 
extern int BTNU_01;
extern int sw6_on_counter;
extern int sw5_on_counter;
extern int sw5_off_counter;
extern int tot_cmds_executed;
extern enum BTNL btnl;
extern int IORegisters[5];
extern int prev_sw0;
extern int prev_sw1;
int sin_on=0;
extern int sw7;
...
int counter=0;
extern int halted;
void __ISR(_TIMER_1_VECTOR, IPL7SAVEALL) Timer1ISR(void)  /* changed to SAVEALL because wasnt compiling with TIMER3 in audio */
{    
	if (j==125){
		do_command=1;
		j=0;
	...
	if (sw7){
		if (BTN_GetValue(2)){ /* BTNC */ 
			if (btnc_timer==0){  
				btnc_timer=1;
				IORegisters[2]++;
                AUDIO_Init(0); /* play sin wave */
                sin_on=1; /* turn flag on */
			}
		}
		else{
			btnc_timer=0;
		}
		if (BTN_GetValue(4)){ /* BTND */ 
			if (btnd_timer==0){  
				btnd_timer=1;
				IORegisters[3]++;
			}
	...
	if (sin_on){ /* indicating the time the sin wave is played */
		sin_on++;
		if (sin_on==4000) { /* turn off after 1 second*/
			AUDIO_Close();
			sin_on=0;
		}
	}
	if (BTN_GetValue(0)){ /* BTNU */
		if (btnu_timer==0){ 
			btnu_timer=1;
			...
			if (sw0==0 && sw1==1){
				...
				if (sw6==1){
                    if (sw6_on_counter==0x1FF){}
                    else{
                        sw6_on_counter++;
                    }
                    sw6_on_counter=sw6_on_counter % (0x1FF);
				}
				else if (sw5==1){
                    if (sw5_on_counter==0x1FF){}
                    else{
                        sw5_on_counter++;
                    }
					sw5_on_counter=sw5_on_counter % (0x1FF);
				}
				else if (sw5==0){
                    if (sw5_off_counter==0x1FF){}
                    else{
                        sw5_off_counter++;
                    }
					sw5_off_counter=sw5_off_counter % (0x1FF);
				}
			}
			else if (sw0==1 && sw1==0){
				BTNU_01++;
	...
	if (BTN_GetValue(1)){ /* BTNL, pause and single step mode */
		if (btnl_timer==0){
			btnl_timer=1;
			AUDIO_Init(0); /* play sin wave */
			sin_on=1; /* turn flag on */
			if (btnl==FLOW){ /* currently in FLOW mode, switch to PAUSE mode*/
				btnl = PAUSE;
			}
			else if (btnl==PAUSE){ /*pause mode */
				btnl = FLOW;
			}
		}
	}
	...
	if (BTN_GetValue(3)){ /*BTNR, execute one command */
		if (btnr_timer==0){
			btnr_timer=1;
			if (btnl==PAUSE && halted==0){ /*only advance in one instruction if currently in PAUSE mode */
